...
for week in range(100):
    for day_offset in range(7):
        run_date = (base_date + timedelta(weeks=week, days=day_offset)).strftime("%Y-%m-%d")
        ...
        # Deliberate weekday seasonality to train the model on day_of_week
        dow_penalty = 0.0
        if dow == 0: dow_penalty = 12.0      # Monday morning rush
        elif dow in [4, 5]: dow_penalty = 25.0 # Friday/Saturday peak travel
        elif dow == 6: dow_penalty = 35.0      # Sunday return congestion
        ...
        for train_num in corridors:
            ...
            if scenario == "ON_TIME_START":
                accum_delay = 0.0 + dow_penalty
            elif scenario == "SLIGHT_DELAY_START":
                accum_delay = (random.uniform(5.0, 15.0) * season_factor) + dow_penalty
            else:
                accum_delay = (random.uniform(25.0, 60.0) * season_factor) + (dow_penalty * 1.5)
            ...
            for idx, st in enumerate(catalog):
                ...
                # Check bottleneck junction keywords (CNB, PRYJ, DDU, GAYA, RTM, BRC, BPL, NGP, BPQ, BZA, KPD, TVC)
                ...
                if is_junction and accum_delay > 0:
                    ...
                    accum_delay += random.uniform(8.0, 22.0) + (dow_penalty * 0.1)
                    ...
                    accum_delay += random.uniform(0.5, 4.0) + (dow_penalty * 0.02)
...
meta = pipeline.train_and_version_model(real_only=False)
